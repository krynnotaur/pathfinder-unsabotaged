...
class SystemTurnur extends AbstractRestController {

    /**
     * cache key for HTTP response
     */
    const CACHE_KEY_TURNUR_CONNECTIONS               = 'CACHED_TURNUR_CONNECTIONS';
    ...
    public function get(\Base $f3){
        $ttl = 60 * 3;
        if(!$exists = $f3->exists(self::CACHE_KEY_TURNUR_CONNECTIONS, $connectionsData)){
            $connectionsData = $this->getEveScoutTurnurConnections();
            $f3->set(self::CACHE_KEY_TURNUR_CONNECTIONS, $connectionsData, $ttl);
        }
     ...
     * @return array
     */
    protected function getEveScoutTurnurConnections() : array {
        $connectionsData = [];
        /**
        ...
        if(!empty($eveScoutResponse) && !isset($eveScoutResponse['error'])){
            foreach((array)$eveScoutResponse['connections'] as $eveScoutConnection){
                if(
                    ...
                    $eveScoutConnection['source']['id'] === 30002086 // Check it's turnur and not a thera connection
                ){
                    try{
                        $data = [
