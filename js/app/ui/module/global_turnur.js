define([
    ...
    let TurnurModule = class TurnurModule extends BaseModule {
        ...
        render(mapId){
            ...
            this._bodyEl.append(
                this.newControlElement(
                    ...
                    [this._config.moduleHeadlineIconClass, this._config.controlAreaTurnurClass, 'hidden'],
                    ['fa-plus']
                )
            );
        ...
        /**
         * init 'Turnur connections' table (EveScout)
         */
        initTurnurTable(){
            let module = this;

            let tableEl = document.createElement('table');
            tableEl.id = module.getTableId('turnur', module._mapId);
            tableEl.classList.add('compact', 'stripe', 'order-column', 'row-border', 'pf-table-fixed', module._config.globalTurnurTableClass);
                            ...
                            cell.addEventListener('click', e => {
                                ...
                                if(rowData.connectionId){
                                    ...
                                    if(systemDataTurnur && systemDataSource){
                                        ...
                                        if(systemDataTurnur){
                                            let systemTurnur = document.getElementById(MapUtil.getSystemId(systemDataTurnur.mapId, systemDataTurnur.id));
                                            if(systemTurnur){
                                                options.sourceSystem = $(systemTurnur);
         ...
         * @param isTurnur
         */
        showNewSystemDialog(options = {}, isTurnur = false){
            let activeMap = BaseModule.Util.getMapModule().getActiveMap();

            // custom callback -> refresh Turnur table
         ...
         * @param systemTurnur
         */
        callbackUpdateTableRows(connectionsData = [], systemTurnur = null){
            if(!this._tableApi){
                return;
            }
            ...
            if(map && currentMapData){
                ...
                // get data from all Turnur connections from current map -----------------------------------------------
                let currentConnectionsData = {};
                let systemTurnurFound = false;
                if(systemTurnur){
                    systemTurnurFound = true;
                    let currentConnections = MapUtil.searchConnectionsBySystems(map, [systemTurnur], '*');
                    currentConnectionsData = BaseModule.getConnectionsDataFromConnections(this._mapId, currentConnections);
                }else{
                    let systemDataTurnur = MapUtil.getSystemData(this._mapId, TurnurModule.systemIdTurnur, 'systemId');
                    if(systemDataTurnur){
                        systemTurnurFound = true;
                        let systemIdTurnur = MapUtil.getSystemId(this._mapId, systemDataTurnur.id);
                        let currentConnections = MapUtil.searchConnectionsBySystems(map, [systemIdTurnur], '*');
                        currentConnectionsData = BaseModule.getConnectionsDataFromConnections(this._mapId, currentConnections);
                    }
                }

                this._bodyEl.querySelector(`.${this._config.controlAreaTurnurClass}`).classList.toggle('hidden', systemTurnurFound);
                ...
                // add mapped Turnur connections (primary) -------------------------------------------------------------
                ...
                // add EveScout Turnur connections (secondary) ---------------------------------------------------------
                ...
                console.warn('Table update failed for Turnur connections. MapData is missing');
            }
        }
        ...
        setModuleObserver(){
            ...
            // add Turnur system
            this._bodyEl.querySelector(`.${this._config.controlAreaTurnurClass}`).addEventListener('click', e => {
                this.showNewSystemDialog({
                    systemData: {
                        id: TurnurModule.systemIdTurnur,
                        name: 'Turnur'
                    }
                }, true);
            }, false);
            ...
            MapUtil.initWormholeInfoTooltip(
                $(this.moduleElement).find('.' + this._config.globalTurnurTableClass),
    ...
    TurnurModule.position = 3;                                       // default sort/order position within sortable area
    TurnurModule.label = 'Turnur';                                   // static module label (e.g. description)
    TurnurModule.systemIdTurnur = 30002086;
    ...
    TurnurModule.defaultConfig = {
        ...
        globalTurnurTableClass: 'pf-global-thera-table',             // class for NPC owned stations table
        controlAreaTurnurClass: 'pf-global-thera-control',           // class for "thera system exists" label

        // fonts
        fontUppercaseClass: 'pf-font-uppercase',                    // class for "uppercase" font
        linkClass: 'pf-link',

        // Turnur connections table
